...
import { Mic, MicOff, Send, Loader2, Target, Settings, Shield, BookOpen, Clock, Activity, ChevronRight, AlertTriangle } from 'lucide-react';
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        if (tasksRes.status === 'fulfilled' && tasksRes.value.length > 0) setTask(tasksRes.value[0]);
        if (machinesRes.status === 'fulfilled' && machinesRes.value.length > 0) setMachine(machinesRes.value[0]);
...
Fuel level: ${machine?.fuel_percent ?? 'UNAVAILABLE'}%
Fuel rate: ${machine?.fuel_rate ?? 'UNAVAILABLE'} L/h
Ground speed: ${machine?.speed ?? 'UNAVAILABLE'} km/h
Engine RPM: ${machine?.engine_rpm ?? 'UNAVAILABLE'}
Engine load: ${machine?.engine_load ?? 'UNAVAILABLE'}%
...
`;
      ...
      console.log(`machine.fuelPercent = ${machine?.fuel_percent}`);
      console.log(`machine.fuelRate = ${machine?.fuel_rate}`);
      console.log(`machine.engineLoad = ${machine?.engine_load}`);
      console.log(`machine.engineRpm = ${machine?.engine_rpm}`);
      ...
      <div className="max-w-[1600px] mx-auto w-full grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_420px] gap-4 lg:gap-8 h-full min-h-0 pb-2 lg:pb-4">
        ...
        <div className="flex flex-col h-full min-h-0 pr-0 lg:pr-8 border-r-0 lg:border-r border-[#141819]">
          ...
          <div className="flex items-center gap-4 mb-6 shrink-0">
            ...
            {errorState && (
              <span className="ml-auto font-mono text-xs tracking-widest text-[#E5484D] uppercase font-bold">
                {errorState}
              </span>
            )}
          </div>
          ...
          <div className="flex-1 min-h-0 flex flex-col overflow-y-auto scrollbar-none pr-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-1 p-3 border border-[#141819]">
                ...
                <span className="font-mono text-xs text-[#F1F3F4] font-bold uppercase truncate">{task?.type || 'UNASSIGNED'}</span>
                <span className="font-mono text-xs text-[#929A9E]">{task?.completion_percent || 0}%</span>
              </div>
