...
@router.get("/training/progress/{operator_id}", response_model=List[schemas.TrainingProgress])
def get_training_progress(operator_id: str, db: Session = Depends(get_db)):
    return db.query(models.TrainingProgress).filter(models.TrainingProgress.operator_id == operator_id).all()
